...
//     );
// };
// to make the component resuable by dynamic rating by user
const Rating = () => {
    const [rating, setRating] = useState(4); // Rating selected by user
    ...
    return (
        <div className="flex items-center gap-2">
            {[...Array(5)].map((_, idx) => (
                <span
                    key={idx}
                    className={`cursor-pointer text-3xl ${idx < rating && "text-yellow-400"}`}
                >
                    <IoStarSharp />
